...
MODEL_DIR = '/nfs/syzhou/github/project/model/'
...
# 下面的变量定义了这些文件的存放地址。
CACHE_DIR = '/nfs/syzhou/github/project/tmp/bottleneck/'

# 图片数据文件夹。
# 在这个文件夹中每一个子文件夹代表一个需要区分的类别，每个子文件夹中存放了对应类别的图片。
INPUT_DATA = '/nfs/syzhou/github/project/flower_data/flower_photos/'
...
# test_per和val_per参数指定了测试数据集和验证数据集的大小。
def create_image_lists(test_per, val_per):
...
def run_bottleneck_on_image(sess, image_data, image_data_tensor, bottleneck_tensor):
    # 这个过程实际上就是将当前图片作为输入计算瓶颈张量的值。这个瓶颈张量的值
    # 就是这张图片的新的特征向量。
...
def get_or_create_bottleneck(sess, image_lists, label_name, index, category, jpeg_data_tensor, bottleneck_tensor):
    ...
    if not os.path.exists(bottleneck_path):
        ...
        # 但却都能通过加载的inception-v3模型生成一个2048的特征向量。具体原理不详。
...
def main(argv=None):
    ...
    # 定义新的神经网络输入，这个输入就是图片进过Inception-3模型向前传播到达瓶颈层时候的输出
    # 可以将这个过程理解为一个特征提取
